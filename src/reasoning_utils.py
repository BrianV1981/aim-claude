...
def execute_ollama(prompt, system_instruction, model, endpoint, timeout=60):
    ...
    try:
        ...
        print(f"[FATAL] Ollama timeout after {timeout}s: {e}", file=sys.stderr)
        return f"FATAL: Ollama provider timed out after {timeout}s. Model may be loading or offline."
